...
def auto_attenuate(start, stop, steps):
    def trigger_and_read(devices, name='primary'):
        ...
        if eiger1m_single in devices:
            ...
            for j in range(16):
                ...
                ret = yield from bps.trigger_and_read([eiger1m_single] + attenuators, 
                    name='auto_scale')
                if len(ret) == 0:
                    break  # simulation mode
                # put in better logic!
                import numpy as np
                if ret[eiger1m_single.stats2.total.name]['value'] > 0 or np.random.rand() > .5:
        ...
        return (yield from bpp.rewindable_wrapper(inner_trigger_and_read(),
                                            rewindable))
    ...
    yield from bp.scan([eiger1m_single] + attenuators, th, start, stop, steps, 
